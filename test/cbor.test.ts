...
const valid_cases = [
    ...
    [`String '"\\'`, [98, 34, 92], `"\\`],
    ["String 'ü' (U+00FC)", [98, 195, 188], "ü"],
    ["String '水' (U+6C34)", [99, 230, 176, 180], "水"],
    ["String '𐅑' (U+10151)", [100, 240, 144, 133, 145], "𐅑"],
    ...
    ["Float32 100000.0", [250, 71, 195, 80, 0], 100000],
    ["Float32 3.4028234663852886e+38", [250, 127, 127, 255, 255], 3.4028234663852886e+38],
    ["Float32 +Infinity", [250, 127, 128, 0, 0], Infinity],
    ["Float32 NaN", [250, 127, 192, 0, 0], NaN],
    ["Float32 -Infinity", [250, 255, 128, 0, 0], -Infinity],
    ["Float64 1.1", [251, 63, 241, 153, 153, 153, 153, 153, 154], 1.1],
    ["Float64 9007199254740994", [251, 67, 64, 0, 0, 0, 0, 0, 1], 9007199254740994],
    ["Float64 1.0e+300", [251, 126, 55, 228, 60, 136, 0, 117, 156], 1e+300],
    ["Float64 -4.1", [251, 192, 16, 102, 102, 102, 102, 102, 102], -4.1],
    ["Float64 -9007199254740994", [251, 195, 64, 0, 0, 0, 0, 0, 1], -9007199254740994],
    ["Float64 +Infinity", [251, 127, 240, 0, 0, 0, 0, 0, 0], Infinity],
    ["Float64 NaN", [251, 127, 248, 0, 0, 0, 0, 0, 0], NaN],
    ["Float64 -Infinity", [251, 255, 240, 0, 0, 0, 0, 0, 0], -Infinity]
];
...
describe("CBOR", function () {
    valid_cases.forEach(c => {
        ...
        it(`decode ${c[0]}`, function () {
            expect(CBOR.decode(bytes, true)).toEqual(json);
        });
        if (json !== undefined) {
            it(`encode ${c[0]}`, function () {
                expect(CBOR.encode(json, true)).toEqual(bytes);
            });
        }
    });
